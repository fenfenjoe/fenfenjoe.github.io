...
环境：JDK1.8+、
使用到的框架：Spring Boot、mockito、JUNIT 5
...
测试粒度不同，单元测试的写法也不同。  
- 使用真实数据：一般使用```@SpringBootTest```注解，加载Spring Boot应用上下文，测试数据库操作。  
  - ```@SpringBootTest```
  - ```@RunWith```
  - ```@Test```
  - ```MockMvc```
  - ```Assert```
- 仅测试当前类中方法的逻辑正确与否：一般使用```@MockBean```注解，模拟依赖注入，测试方法中使用```@Mock```注解模拟依赖对象。
