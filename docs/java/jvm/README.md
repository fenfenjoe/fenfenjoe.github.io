...
```
操作系统进程（JVM 进程）虚拟地址空间
│
├── 代码段 & 数据段
│   ├── JVM 可执行代码（java 命令）
│   ├── 共享库（libjvm.so、libc.so 等）
│   ├── 已初始化全局/静态变量
│   └── 未初始化全局/静态变量（BSS）
│
├── 堆（Heap）—— 由 JVM 管理
...
│   │   ├── 老年代
│   │   └── 存储内容：对象实例、字符串常量池（JDK 7+）、静态变量（JDK 7+）
│   └── （部分 JVM 实现可能将堆分配在内存映射区，但逻辑上仍是堆）
│
├── 内存映射区（mmap 区域）
│   ├── 元空间（Metaspace，JDK 8+）
│   │   └── 类元数据、运行时常量池
...
│   │   ├── 本地方法栈（Native 方法栈）
│   │   └── 程序计数器（线程私有，逻辑上位于此处）
│   └── 其他映射文件（如 JAR 文件、类文件等）
│
└── 栈（Stack）—— 主线程栈（通常较小，其余线程栈在上面的 mmap 区域）
    └── 主线程的调用栈（如果 JVM 未使用独立线程栈区域，则可能混用）
```
