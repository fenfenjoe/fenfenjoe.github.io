...
在 Vue 应用中，每个组件都有自己的局部状态（`data`）。当多个组件需要共享同一份数据时，就会遇到以下问题：

- **兄弟组件通信**：需要通过父组件中转，逐层传递繁琐
- **跨层级通信**：深层嵌套组件间传递数据，导致 `props` 链路冗长（prop drilling）
- **数据同步**：多处修改同一数据，难以追踪变更来源，容易产生不一致

**状态管理**的核心思想是将这些**共享状态**从组件中抽离出来，放入一个全局的、集中的"仓库"（Store）中统一管理。任何组件都可以读取 store 中的状态，任何需要修改状态的操作也必须通过规范的方式进行，从而使数据流向可预测、可追踪、易调试。

```
没有状态管理：组件 A ←→ 组件 B ←→ 组件 C（点对点通信，混乱）

引入状态管理：组件 A ↘
              组件 B → Store（单一数据源，规范流转）
              组件 C ↗
```

Vuex 是 Vue2 官方的状态管理方案，它借鉴了 Flux 架构，通过严格的单向数据流约束，确保状态变更有据可查。
